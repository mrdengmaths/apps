...
class GameController {
    ...
    setupEventListeners() {
        ...
        // Listen for answer selection from MCQ buttons
    ...
    checkAnswer() {
        ...
        const questionFormat = this.state.currentQuestion.format || 'math';
        let isCorrect = false;

        if (questionFormat === 'mcq') {
            isCorrect = String(userAnswer).trim() === String(correctAnswer).trim();
        } else if (questionFormat === 'text') {
            isCorrect = this.normalizeTextAnswer(userAnswer) === this.normalizeTextAnswer(correctAnswer);
            ...
            isCorrect = Number.isFinite(userNum) && Number.isFinite(correctNum) && Math.abs(userNum - correctNum) < 1e-6;
        } else {
            isCorrect = this.algebraEngine.compareExpressions(userAnswer, correctAnswer, this.state.currentLevel.value);
        }
        ...
        if (isCorrect) {
            ...
            if (this.state.isSecondIncorrectAttempt()) {
                ...
                this.ui.showFeedback(false, null, correctAnswer, questionFormat);
                this.timer.reset();
